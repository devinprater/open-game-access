...
def main():
    ...
    def gated(button, want, tries=20):
        for _ in range(tries):
            # Triple-send: the debugger input queue drops single presses often;
            # three back-to-back sends land at least one hold almost every time.
            for _ in range(3):
                pad.ws.send(json.dumps({"event": "input.buttons.press", "requestId": 1,
                                        "button": button, "frames": 120}))
                time.sleep(0.2)
            seen = []
            for _ in range(6):
                v = safe_u32(PAD)
    ...
    print("=== survivors: index CHANGED with display, count STABLE ===",
          flush=True)
    ccmap = dict(calm)
    surv = []
    for ad in ccmap:
